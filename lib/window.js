...
export class WindowManager extends GObject.Object {
    ...
    place(where) {
        ...
        // we need to grab this each time in case it's changed
        const secondarySplit = this.settings.get_double('secondary-split');
        Logger.debug('WindowManager:place secondarySplit', secondarySplit);
        const aHeight = height * secondarySplit;
        const bHeight = height * (1.0 - secondarySplit);
        Logger.debug(
        ...
        // Remove any maximization so move_resize_frame can freely reposition
        focused.unmaximize();
